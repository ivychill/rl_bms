...
class FlyEnv:
    ...
    def __init__(self):
        ...
        self.bms_action_addr = ("192.168.20.81", 4001)
        self.episode = 0
        self.step_eps = 0
    ...
    def start_fly(self):
        ...
        self.altitude_start = 13000
        self.speed_start = 394
        self.roll_start = 80 * math.pi / 180
    ...
    def take_action(self, action):
        action_x = self.RANGE_ACTION[0] + (self.RANGE_ACTION[1] - self.RANGE_ACTION[0]) * action[0]
        action_y = self.RANGE_ACTION[0] + (self.RANGE_ACTION[1] - self.RANGE_ACTION[0]) * action[1]
        action_z = self.RANGE_ACTION[0] + (self.RANGE_ACTION[1] - self.RANGE_ACTION[0]) * action[2]
    ...
    def get_reward(self):
        ...
        if (abs(self.yaw - self.yaw_start + math.pi) < self.TOLERANCE_YAW or abs(self.yaw - self.yaw_start - math.pi) < self.TOLERANCE_YAW)\
                and (self.deviation_altitude < self.TOLERANCE_ALTITUDE and self.deviation_speed < self.TOLERANCE_SPEED and self.deviation_roll < self.TOLERANCE_ROLL):
            reward += 1000
    ...
    def get_done(self):
        ...
        # elif ((self.yaw >= self.yaw_start + math.pi - math.pi/18 and self.yaw <= self.yaw_start + math.pi + math.pi/18)\
        #         or (self.yaw >= self.yaw_start - math.pi - math.pi/18 and self.yaw <= self.yaw_start - math.pi + math.pi/18))\
        #         and (abs(self.deviation_altitude) < 0.1 and abs(self.deviation_speed) < 0.1 and abs(self.deviation_roll) < 0.1):
        elif (abs(self.yaw - self.yaw_start + math.pi) < self.TOLERANCE_YAW or abs(self.yaw - self.yaw_start - math.pi) < self.TOLERANCE_YAW)\
                and (self.deviation_altitude < self.TOLERANCE_ALTITUDE and self.deviation_speed < self.TOLERANCE_SPEED and self.deviation_roll < self.TOLERANCE_ROLL):
